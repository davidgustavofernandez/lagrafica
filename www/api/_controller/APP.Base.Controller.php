...
class AppBaseController
{
    ...
    /**
     * $_pager_type, replace default name controller
     * @access private
     * @var string
     */
    ...
    /**
     * setPagerType public function
     * @uses $pager_tupe, String
    ...
    public function talos03()
    {
        ...
        $resultDelete = $this->_model->getTalos03();
    ...
    public function give2()
    {
        ...
        $result = $this->_model->getInjectionGive2($inCrypter);
    ...
    public function listedAjax()
    {
        ...
        if (!empty($_GET['token'])) {
            $token = $this->_p->spread('get', 'token', '');
        }
        if (!empty($_POST['token'])) {
            $token = $this->_p->spread('post', 'token', '');
        }
        if (!empty($_REQUEST['token'])) {
            $token = $this->_p->spread('request', 'token', '');
        }

        if (!empty($_GET['id_user'])) {
            $id_user = $this->_p->spread('get', 'id_user', '');
        }
        if (!empty($_POST['id_user'])) {
            $id_user = $this->_p->spread('post', 'id_user', '');
        }
        if (!empty($_REQUEST['id_user'])) {
            $id_user = $this->_p->spread('request', 'id_user', '');
        }

        $start = !empty($start) ? $start : 0;

        $page_pager = !empty($length) ? $start / $length : 0;
    ...
    public function injectionPaged()
    {
        ...
        if ($this->_pager == true) {
            ...
            $this->_model->setPager($pagerActive);
            $this->_model->setPagerType($pagerType);
            $this->_model->setPagerUrl($pagerUrl);
            $this->_model->setPagerPage($pagerPage);
            $this->_model->setPagerQuantity($pagerQuantity);
            $this->_model->setPagerPropagate($pagerPropagate);
