...
class AppController{
  ...
  public function apiStart(){
    ...
    $this->_pager_type = $p->spread('request','pager_type','');
    ...
    if(!empty($_REQUEST['search_on'])){
      ...
      $this->_crypter = new Crypt();
      $this->_crypter->setKey(CRYPT_VAR_TXT);
      $temp_search_on = $this->_crypter->getDecrypt($p->spread('request','search_on',''));
    ...
    if(!empty($_REQUEST['search_off'])){
      ...
      $this->_crypter = new Crypt();
      $this->_crypter->setKey(CRYPT_VAR_TXT);
      $temp_search_off = $this->_crypter->getDecrypt($p->spread('request','search_off',''));
    ...
    if(!empty($_REQUEST['search_filters'])){
      ...
      $this->_crypter = new Crypt();
      $this->_crypter->setKey(CRYPT_VAR_TXT);
      $temp_search_filters = $this->_crypter->getDecrypt($p->spread('request','search_filters',''));
    ...
    if(!empty($_REQUEST['params_filter'])){
      ...
      $this->_crypter = new Crypt();
      $this->_crypter->setKey(CRYPT_VAR_TXT);
      $temp_params_filter = $this->_crypter->getDecrypt($p->spread('request','params_filter',''));
    ...
    $_key_cross_site = $p->spread('request','public_key',''); # Public Application Key for API
    $_key_cross_site_key = $p->spread('request','private_key',''); # Private Aplication Key to be able to modify data of the administrator entity
    $_key_cross_site_api_token = $p->spread('request','personal_key',''); # Personal Key of Administrator
    ...
    $control->setPagerType($this->_pager_type);
