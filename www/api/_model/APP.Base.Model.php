...
class AppBaseModel extends AppAbstractConnectInstance
{
    ...
    /**
     * $_search_off, Array with fields where the search is applied
     * @access private
     * @var array
     */
    ...
    public function getRecords()
    {
        ...
        $cl->setPagerType($this->_pager_type);
    ...
    public function getInjectionGive2($query)
    {
        if (!empty($query)) {
            $cl = new QueryMethods();
            $cl->setConn($this->_conn);
            $resultSelect = $cl->injection2($query);
            $estadoSelect = $cl->state();
            return $resultSelect;
        } else {
            return 'error';
