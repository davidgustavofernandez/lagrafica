...
class Rules{
    ...
    private function _setItems(){
        ...
        'tablas' => array(
                array(
                    'relacion' => 'user_availability_treatment',
                    'tabla_a' => $this->_prefix.'users_availabilities_treatments',
                    'tabla_b' => $this->_prefix.'users_availabilities_treatments_relations',
                    'tabla_c' => $this->_prefix.'users_availabilities',
                    'id_a' => 'id_user_availability_treatment',
                    'id_b' => 'id_user_availability_treatment_relation',
                    'id_c' => 'id_user_availability',
                    'id_filtro' => 'id_user_availability'
                )
            )
        );
    }
    ...
    private function _setQuerys(){
        $this->_querys = array(
            ...
            'preview' => 'SELECT p.[id_a] as id, p.name, p.minutes,(CASE WHEN ap.[id_a] is null then 0 else 1 end ) as chequeado, p.status FROM [tabla_a] p LEFT JOIN [tabla_b] ap on ap.[id_a] = p.[id_a] AND ap.[id_c]="[[id_c]]" AND ap.status="1" AND p.status="1" GROUP BY p.[id_a] order by p.field_order',
