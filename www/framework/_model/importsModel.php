<?php
/**
 * importsModel, Model "base"
 *
 * The importsModel Class is the "base" model to mediate between the controller and the view by applying the model. Interact with the Database
 ...
 */
class importsModel extends AppAbstractConnectInstance{
    /**
    ...
    public function getRecordsPagedAjaxFull(){
      ...
      /**
       * getTalosImport public function
       * Note: It treats the data of a query of type ID and inserts the data, in addition it treats data passed by GET, POST and REQUEST. It deals with the data posted. Use to Insert.
       */
      // public function getTalosImport(){
      //   // HARCODEO, We gonna use another controller!
      //   $referenced_controller = 'imports';
      //   $cl = new QueryMethods();
      //   $cl->setConn($this->_conn);
      //   $cl->setTable($this->_table);
      //   $cl->setFields($this->_fields);
      ...
      public function runImport(){
        ...
        // GET parameters by $_GET
        if( !empty($this->_fields) && is_array($this->_fields) && !empty($_GET) && is_array($_GET) ){
          $posteos_names = array_keys($_GET);
          $posteos_values = array_values($_GET);
          $namesPostLength = sizeof($posteos_names);
          $namesArray = array();
  
          for($i=0; $i<$namesPostLength; $i++){
            if( in_array($posteos_names[$i],$this->_fields) ){
              $namesArray[$posteos_names[$i]] = $posteos_values[$i];
            }
          }
        ...
        // Get data record from Imports
        $data_table_import = $this->getInjection("SELECT * FROM " . $this->_table . " WHERE " . $this->_key . "='" . $idAplicarValue . "' and status=1");
        $data_table_import_encode = json_encode($data_table_import);
        $data_table_import_decode = json_decode($data_table_import_encode, true);

        // print_r($data_table_import_decode);
        $truncate_table = !empty($data_table_import_decode[0]['ttable']) ? true : false;
        $import_file = $data_table_import_decode[0]['file'];
        $import_id_import_model = $data_table_import_decode[0]['id_import_model'];
        $import_id_import_character = $data_table_import_decode[0]['id_import_character'];
        
        // GET data record from import_nodels
        $data_table_import_model = $this->getInjection("SELECT * FROM " . $this->_table . "_models WHERE id_import_model='" . $import_id_import_model . "' and status=1");
        $data_table_import_model_encode = json_encode($data_table_import_model);
        $data_table_import_model_decode = json_decode($data_table_import_model_encode, true);
        $import_model_table = $data_table_import_model_decode[0]['table_name'];
        
        // echo $truncate_table."\n";
        // echo $import_model_table."\n";
        // exit();
        // echo "TRUNCATE TABLE " . CONFIG_DB_PREFIX.$data_table_import_model_decode[0]['table_name'] . ";";
        // exit();

        if($truncate_table==true){
            $data_table_import = $this->getInjection("TRUNCATE TABLE " . CONFIG_DB_PREFIX.$data_table_import_model_decode[0]['table_name'] . ";");
        }

        $import_files_path_physical = dirname(__FILE__) . '/../../material/imports/media/'.$import_file;

        if(file_exists($import_files_path_physical)){
          $the_file = $import_files_path_physical;

          // GET structure database
          $estructuraBbdd = $this->getStructureDatabase();
          $foreing_table_fields = $estructuraBbdd[CONFIG_DB_NAME][CONFIG_DB_PREFIX.$import_model_table]['GLOBAL_data'];
          $foreing_controller_list_of_fields = array();
          $foreing_controller_id = '';
          foreach($foreing_table_fields as $kk => $vv){
              $foreing_controller_list_of_fields[]=$kk;
              if($vv['key']=='PRI'){
                $foreing_controller_id = $kk;
              }
          }

          $importer = $this->setMakeImpport($the_file, 'imports', $import_model_table, $foreing_controller_list_of_fields,$foreing_controller_id,$import_id_import_character);

          if(!empty($importer) && is_integer($importer)){
            $returned = array(
              'total' => $importer,
              'controller' => $import_model_table
            );
            return $returned;
          }else{
            $returned = array(
      ...
      public function setMakeImpport($file, $controller, $foreing_controller, $foreing_fields, $foreing_id, $id_import_character)
      {
        $count_insert_fields = 0;

        if(!empty($file) && !empty($controller))
        {          
          if(file_exists($file))
          {
            $path = dirname(__FILE__) . '/../_lib/import/Class.Import.php';
            require_once($path);
            $import = new Import();
            $import->setFile($file);
            $import->setDelimiter(",");
            ...
            $foreingFieldsLength = sizeof($foreing_fields)-1;

            if(is_array($datos) && !empty($datos[0][0]))
            {
              $dataLength = sizeof($datos);
              
              for($i=0; $i<$dataLength; $i++){
                if($i>=1)
                {
                  $query = 'INSERT INTO '.CONFIG_DB_PREFIX.$foreing_controller.' SET ';

                  foreach($foreing_fields as $k => $v){
                    if($k != 0) // IS te ID of table don use this data
                    {
                      if($k == $foreingFieldsLength)
                      {
                        if($id_import_character=='2') {
                          $query .= !empty($datos[$i][$k]) ? $foreing_fields[$k] . "='" . $datos[$i][$k] . "'; " : "";
                        } elseif ($id_import_character=='3') {
                          $query .= !empty($datos[$i][$k]) ? $foreing_fields[$k] . "='" . $datos[$i][$k] . "'; " : "";
                        } else {
                          $query .= !empty($datos[$i][$k]) ? $foreing_fields[$k] . "='" . $datos[$i][$k] . "'; " : "";
                        }
                ...
                //   echo $query;
                //   exit();
                  $return_query = $this->getInjection(str_replace('¶',' ',$query));
                  if ($this->_estadoSelect != 'impact') {
                    echo $query . "\n";
                    echo $this->_estadoSelect . "\n";
                    ob_flush();
                    flush();
                  }
                  
                  // exit();
                  $count_insert_fields = $count_insert_fields + 1;
                }
              }
            }
          }
        }
        return $count_insert_fields;
      }
    ...
    */
    public function getInjection($query){
        if(!empty($query)){
            ...
            $estadoSelect = $cl->state();
            $this->_estadoSelect = $estadoSelect;
