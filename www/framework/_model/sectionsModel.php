<?php
/**
 * sectionsModel, Model "base"
 *
 * The sectionsModel Class is the "base" model to mediate between the controller and the view by applying the model. Interact with the Database
 ...
 */
class sectionsModel extends AppAbstractConnectInstance{
    /**
    ...
    public function getMakeFile(){
        if($this->_save_data_on_file==true){
            if(in_array('field_order',$this->_fields)){
                ...
                if($this->_table=='maa_administrators'){
                    $data_table = $this->getInjection("SELECT id_administrator, api_token, access_api FROM ".$this->_table." WHERE status='1'");
                }else{
            ...
            if(!empty($data_table)){
                ...
                $data_table_encode = json_encode($data_table);
                $data_table = json_decode($data_table_encode, true);

                $sections_childs_array = array();
                $sections_childs_array_count = count($data_table);
                for($i=0; $i<$sections_childs_array_count; $i++){
                  if($data_table[$i]['menu_header']=='1'){
                    if(!empty($data_table[$i]['parent_id_section'])){
                      if($data_table[$i]['show_children_header']=='1'){
                        if(is_array($sections_childs_array[$data_table[$i]['parent_id_section']]['childs'])){
                          $sections_childs_array[$data_table[$i]['parent_id_section']]['childs'][$data_table[$i]['id_section']] = $data_table[$i];
                        }else{
                          $sections_childs_array[$data_table[$i]['parent_id_section']]['childs'] = array();
                          $sections_childs_array[$data_table[$i]['parent_id_section']]['childs'][$data_table[$i]['id_section']] = $data_table[$i];
                        }
                      }
                    }else{
                      $sections_childs_array[$data_table[$i]['id_section']] = $data_table[$i];
                      $sections_childs_array[$data_table[$i]['id_section']]['childs'] = '';
                    }
                  }
                }

                $path_childs = dirname(__FILE__).'/../../material/_data/'.$this->_table.'__custom_nav.inc';

                if(file_exists($path_childs)){
                    @unlink($path_childs);
                }

                $dump_fp_childs = fopen($path_childs,"w");

                if($dump_fp_childs != false){
                    fwrite($dump_fp_childs, serialize($sections_childs_array));
                    fclose($dump_fp_childs);
                }
            }
            ...
            if(!empty($data_table)){
                ...
                $sections_full_childs_array = array();
                $sections_full_childs_array_count = count($data_table);
                for($i=0; $i<$sections_full_childs_array_count; $i++){
                  if($data_table[$i]['menu_header']=='1'){
                    if(!empty($data_table[$i]['parent_id_section'])){
                      // if($data_table[$i]['show_children_header']=='1'){
                        if(is_array($sections_full_childs_array[$data_table[$i]['parent_id_section']]['childs'])){
                          $sections_full_childs_array[$data_table[$i]['parent_id_section']]['childs'][$data_table[$i]['id_section']] = $data_table[$i];
                        }else{
                          $sections_full_childs_array[$data_table[$i]['parent_id_section']]['childs'] = array();
                          $sections_full_childs_array[$data_table[$i]['parent_id_section']]['childs'][$data_table[$i]['id_section']] = $data_table[$i];
                        }
                      // }
                    }else{
                      $sections_full_childs_array[$data_table[$i]['id_section']] = $data_table[$i];
                      $sections_full_childs_array[$data_table[$i]['id_section']]['childs'] = '';
                    }
                  }
                }

                $path_childs = dirname(__FILE__).'/../../material/_data/'.$this->_table.'__custom_full.inc';
                ...
                if($dump_fp_childs != false){
                    fwrite($dump_fp_childs, serialize($sections_full_childs_array));
                    fclose($dump_fp_childs);
                }
            }
        }
