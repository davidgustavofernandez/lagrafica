...
# THIS ALWAYS GOES {
    ...
    /**
     * $_search_on, Array with fields where the search is applied
     * @access private
     * @var array
     */
     ...
     * Note: Array with fields where the search is applied
     */
...
# } THIS ALWAYS GOES
    ...
    public function seeker()
	{
        ...
        $this->_model->setStatus($this->_status);
        $this->_model->setTable($this->_prefix.$this->_table);
        $this->_model->setSearchOn($this->_search_on);
        $this->_model->setSearchOff($this->_search_off);	
        $this->_model->setSearchFilters($this->_search_filters);
        $this->_model->setFields($this->_fields);
		// $arrayFields = $this->_fields;
        // $this->_model->setFieldAppliedArray($arrayFields);
        $result = $this->_model->getSeeker();
        // print_r($this->_params_filter);
        // exit();
    ...
    public function exportBatch(){
        ...
        $_REQUEST['amount'] = $this->_p->spread('request','amount',5000);
        ...
        $resultExportBranch = $this->_model->getExportBatch();
        $table_name = $this->_estructuraBbdd[$this->_database][$this->_prefix.$this->_table]['GLOBAL_alternatename'];
        // $table_show_in_list = $this->_estructuraBbdd[$this->_database][$this->_prefix.$this->_table]['GLOBAL_showinlist'];
        // $table_show_in_form = $this->_estructuraBbdd[$this->_database][$this->_prefix.$this->_table]['GLOBAL_showinform'];
        $table_save_data_on_file = $this->_estructuraBbdd[$this->_database][$this->_prefix.$this->_table]['GLOBAL_save_data_on_file'];
    ...
    /**
     * exportBatchList public function
     * Note: List all files on 'reports' folder
     */
    public function exportBatchList(){
        ...
        $resultExportBatchList = $this->_model->getExportBatchList();
    ...
    /**
     * ExportBatchFileDelete public function
     * Note: Delete one spesific file on 'reports' folder
     */
    public function exportBatchFileDelete(){
        if(!empty($_GET['file'])){
        ...
        if(!empty($file)){
            $this->_model->setController($this->_controller);
            $resultExportBatchFileDelete = $this->_model->getExportBatchFileDelete($file);
        }else{
            $resultExportBatchFileDelete = false;
        }
    ...
    /**
     * ExportBatchEraseAllFiles public function
     * Note: Delete all files on 'reports' folder
     */
    public function exportBatchEraseAllFiles(){

        $this->_model->setController($this->_controller);
        $resultEraseAllExportBatch = $this->_model->getExportBatchEraseAllFiles();
