<?php
/**
 * importsController, Controller "base"
 * 
 * The Class importsController is the "base" controller responsible for processing the requests.
...
# THIS ALWAYS GOES {
    ...
    public function getModel(){
        $path = dirname(__FILE__) . '/../'.FOLDER_MODEL.'importsModel.php';
    ...
    public function getView(){
        $path = dirname(__FILE__) . '/../'.FOLDER_VIEW.'importsView.php';
...
# } THIS ALWAYS GOES
    ...
    public function talos01(){
        ...
        if(!empty($_REQUEST['id_import_model'])){ 
            $foreingModel = $this->_p->spread('request','id_import_model',0);
            $foreingModelData = $this->_model->getInjection("SELECT table_name FROM ".$this->_prefix."imports_models WHERE id_import_model=".$foreingModel." ");
            if(!empty($foreingModelData)){
                $foreingModelData_encode = json_encode($foreingModelData);
                $foreingModelData_decode = json_decode($foreingModelData_encode, true);
                // echo $foreingModelData_decode[0]['table_name'];
                $foreing_table_fields = $this->_estructuraBbdd[$this->_database][$this->_prefix.$foreingModelData_decode[0]['table_name']]['GLOBAL_data'];
                // print_r($foreing_table_fields);
                $foreing_controller_list_of_fields = array();
                foreach($foreing_table_fields as $k=> $v){
                    $foreing_controller_list_of_fields[]=$k;
                }
            }
            // exit();
        };
        ...
        if(!empty($resultInsertar)){
            ...
            $temp_return_upload = explode('|',$return_upload);
            // // echo $temp_return_upload['3'];
  
            // if(!empty($temp_return_upload['3'])){
            //     $the_file = $temp_return_upload['3'];
            //     $table_fields = $this->_estructuraBbdd[$this->_database][$this->_prefix.$this->_table]['GLOBAL_data'];
                
            //     $controller_list_of_fields = array();
            //     foreach($table_fields as $key => $value){
            //         $controller_list_of_fields[]=$key;
            //     }
            //     // print_r($controller_list_of_fields);
            //     // exit();
            //     $this->_model->setMakeImpport($the_file, $this->_table, $controller_list_of_fields, $foreingModelData_decode[0]['table_name'], $foreing_controller_list_of_fields, $resultInsertar);
            // }
        }
