...
class Validate extends ArrayObject {
	...
	public static function not_empty($value)
	{
		...
		// Value cannot be NULL, FALSE, undefined, '', or an empty array
		return ! in_array($value, array(NULL, FALSE, 'undefined', '', array()), TRUE);
	}
	...
	public static function validateDate($str, $format = 'Y-m-d H:i:s')
	{
		$d = DateTime::createFromFormat($format, (string) $str);
		$result = $d && $d->format($format) == (string) $str;
		return (bool) $result;
	...
	public static function validateTime($str)
	{
		$str = (string) $str;
		return (bool) preg_match('/^(?:2[0-3]|[01][0-9]):[0-5][0-9]:[0-5][0-9]$/', $str);
	}
	...
	public static function validateArrayLength($data)
	{
		if(is_array($data)){
			$total = count($data);
			if($total<=0){
	...
	// Messages list
	protected $_errors_messages = array();
	...
	{
		include 'Class.Validate.Seting.php';

		$this->_errors_messages = $config['v_errors'];
	...
	public function errors($file = NULL)
	{
		...
		foreach ($this->_errors as $field => $set)
		{
			...
			$message = (isset($this->_errors_messages[$field][$error])) ? $this->_errors_messages[$field][$error] : $this->_errors_messages[$field]['default'];
