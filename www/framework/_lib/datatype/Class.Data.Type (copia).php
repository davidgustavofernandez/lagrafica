                ...
                case 'radioselect':
                    ...
                    if( is_array($retorno) )
                    {
                        ...
                        for ($i=0; $i<$lengDatos; $i++) //loop de los campos
                        {
                            ...
                            $combo .= htmlentities(ucwords($retorno[$i]->name)) . '<br>';
                        }
                        $combo .= '';
                        $this->_result = $combo;
                    }
                ...
                case 'select':
                    ...
                    if( is_array($retorno) )
                    {
                        ...
                        for ($i=0; $i<$lengDatos; $i++) //loop de los campos
                        {
                            //print_r($retorno[$i]);
                            $key_foraneo = $this->_typeArray['KEY'];
                            $last_name = !empty($retorno[$i]->last_name) ? ' '.htmlentities(ucwords($retorno[$i]->last_name)) : '';
                            if($retorno[$i]->$key_foraneo==$value){
                                if(!empty($retorno[$i]->id_idioma)){
                                    $combo .= "<option value='".$retorno[$i]->$key_foraneo."' selected=selected>".$retorno[$i]->name . $last_name . " - ".$retorno[$i]->id_idioma."</option>"."\n";
                                }else{
                                    $combo .= "<option value='".$retorno[$i]->$key_foraneo."' selected=selected>".$retorno[$i]->name . $last_name . "</option>"."\n";
                                }
                            }else{
                                if(!empty($retorno[$i]->id_idioma)){
                                    $combo .= "<option value='".$retorno[$i]->$key_foraneo."' >".$retorno[$i]->name . $last_name . " - ".$retorno[$i]->id_idioma."</option>"."\n";
                                }else{
                    ...
                    if( is_array($retorno) )
                    {
                        ...
                        for ($i=0; $i<$lengDatos; $i++) //loop de los campos
                        {
                            if($retorno[$i]->status>=0)
                            {
                                ...
                                $idioma = !empty($retorno[$i]->id_idioma) ? $retorno[$i]->id_idioma : '';

                                $combo .= '<input type="checkbox" name="' . $this->_typeArray['NAME'] . '[]" value="'.$retorno[$i]->id.'" '.$is_cheked.' class="multicheckbox"  /> ';
                                $last_name = !empty($retorno[$i]->last_name) ? ' '.htmlentities(ucwords($retorno[$i]->last_name)) : '';
                                $combo .= $idioma . ' ' . htmlentities(ucwords($retorno[$i]->name)) . $last_name . '<br>';
                            }
                        }
                        $combo .= '';
                        $this->_result = $combo;
    ...
    public function _show()
    {
        if ( isset($this->_typeArray['TYPE']) && isset($this->_typeArray['NAME']) && isset($this->_typeArray['ID']) && isset($this->_typeArray['VALUE']))
        {
            ...
            switch($this->_typeArray['TYPE'])
            {
                ...
                break;
                case 'varcharlink':	
                    $this->_result = '<a href="' . $value . '" target="_blank">Ver</a>';
                break;
                case 'char':			
                    $this->_result = '' . $value . '';	
                break;
                ...
                case 'radioselect':
                    ...
                    if( is_array($retorno) )
                    {
                        ...
                        for ($i=0; $i<$lengDatos; $i++) //loop de los campos
                        {
                            ...
                            $last_name = !empty($retorno[$i]->last_name) ? ' '.htmlentities(ucwords($retorno[$i]->last_name)) : '';
                            $combo .='<input name="' . $this->_typeArray['NAME'] . '" id="'.$retorno[$i]->$key_foraneo.'" type="radio" value="'.$retorno[$i]->$key_foraneo.'"'.$chequeado.' class="form-control radioselect" />';
                            $combo .= htmlentities(ucwords($retorno[$i]->name)) . $last_name .'<br>';
                        }
                        $this->_result = $combo;
                    }
                break;
                ...
                case 'multicheckbox':
                    ...
                    if( is_array($retorno) )
                    {
                        ...
                        for ($i=0; $i<$lengDatos; $i++) //loop de los campos
                        {
                            ...
                            $combo .= '<input type="checkbox" name="' . $this->_typeArray['NAME'] . '[]" value="'.$retorno[$i]->id.'" '.$is_cheked.' class="multicheckbox"  /> ';
                            $last_name = !empty($retorno[$i]->last_name) ? ' '.htmlentities(ucwords($retorno[$i]->last_name)) : '';
                            $combo .= $retorno[$i]->id.' ' . htmlentities(ucwords($retorno[$i]->name)) . $last_name . '<br>';
                        }
                        $combo .= '';
                        $this->_result = $combo;
                    }
                break;
                case 'url':			
                    $this->_result = '<a href="' . htmlentities($value) . '" target="_blank">Ver.</a>';	
                break;
                default:
                    //
                break;
                ...
                case 'radioselect':
                    ...
                    if( is_array($retorno) ){
                        ...
                        for ($i=0; $i<$lengDatos; $i++){
                            $key_foraneo = $this->_typeArray['KEY'];
                            $combo .= htmlentities(ucwords($retorno[$i]->name));
                        }
                        $this->_result = $combo;
                    }
                break;
