  ...
  if( is_array($resultSelect) ){
    ...
    foreach($resultSelect as $record){
      ...
      if($record->$the_status!=3){
          ...
          if($value!=$the_status && $value!=$the_order){
            ...
            if(in_array($value,$this->_fields_relation) && !empty($table_fields[$value]['query']) ){
              ...
              if($table_fields[$value]['type_of']=='file' && !empty($record->$value)){
                ...
                $return_list .= '   <div class="strings rx'.$quantity_fields.'">' . $record->$value . ' <a href="' . $record->$value . '" target="_blank">[ver]</a></div>';
              }else{
                # email
        ...
        $return_list .= '       <a href="index.php?controller='.$this->_controller.'&amp;action=migrate&amp;'.$the_key.'='.$record->$the_key.'&amp;" class="bot_action btn-danger fa fa-upload" title="Migrate" onclick="return confirm(\'Are you sure you want to MIGRATE this to PRODUCTION?\');"></a>';        
        $return_list .= '   </div>';
