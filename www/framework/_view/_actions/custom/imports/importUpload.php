<?php
/* View: import
 * Action: Create the upload form
 * Return: array() return
 * Comment: Create the upload form to send the data to the talosImport driver
 */
...
foreach($this->_estructuraBbdd[$this->_database][$this->_prefix.$referenced_controller]['GLOBAL_data'] as $key=> $value){
    ...
    if($field_primary_key!='PRI' && $field!=$this->_status){
        if($field_form=='true'){
            if(!empty($table_fields[$key]['query'])){
                if($field_type_of=='multicheckbox'){
            ...
            if(!empty($this->_key) && $field==$this->_order){
                $the_max_id = $this->_model->getInjection(' SELECT MAX('.$this->_key.') as the_max_id FROM '.$this->_prefix.$referenced_controller.' ;');
                $the_max_id = empty($the_max_id[0]->the_max_id) ? 1 : ($the_max_id[0]->the_max_id) + 1 ;  
                $values = $the_max_id;
            }
            // echo $field_type_of.'|'.$field . '|'.$values.'<br>';

            // Harcodeo de name
            if($field=='name' && $field_type_of=='varchar')
            {
              $typeIn = array(
                  'TYPE' => 'readonly',
                  'NAME' => $field,
                  ...
                  'TABLE' => $referenced_controller,
                  'QUERY_PREPARED' => $field_primary_query,
                  'FORM' => $referenced_controller,
            ...
            if($field_type_of=='file'){
                ...
                $html_field .= '    <div class="col-sm-10">'."\r";
...
$html_main .= '              <input type="hidden" name="action" value="talosImport" />';
...
$html_main .= '                      <button type="button" class="btn btn-primary" onClick="validar();">'.$this->_m->getMessageEntites("view_form","save").'</button>';
...
$ckeditor_script = '
    ...
    CKEDITOR.config.removeButtons = "Subscript,Superscript,Save,NewPage,Preview,Templates,Print";
