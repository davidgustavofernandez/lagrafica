<?php
/* View: edit
 * Action: Show the edit form for a record with content
 * Return: HTML return
 * Comment: Generate a form with the content
 */
...
$id_record_name = $resultEdit['1'];
$id_record_value = $resultEdit['0']->$id_record_name;
$html_field = '';
...
foreach($this->_estructuraBbdd[$this->_database][$this->_prefix.$this->_table]['GLOBAL_data'] as $key=> $value){
    ...
    // echo $field_type_of . '|'.$field_primary_key.'<br>';

    if($field_primary_key!='PRI' && $field!=$this->_status && $field!='modified'){
        if($field_form=='true'){
            $search_in = '';
            $default_valor_name = '';
            if(!empty($table_fields[$key]['query'])){
                if($field_type_of=='multicheckbox' || $field_type_of=='multicheckboxchild') {
                    $pre_field_primary_query = str_replace('['.$id_record_name.']', $resultEdit['0']->$id_record_name, $this->_rules_instructions[$field]['edit']);
                    $field_primary_query = !empty($pre_field_primary_query) ? $this->_model->getInjection($pre_field_primary_query) : 'error';
                }elseif($field_type_of=='seeker'){
                    $_y = new Crypt();
                    $temp_search_in = base64_encode(serialize($field_foreign_fields));
                    $_y->setKey(CRYPT_VAR_TXT);
                    $search_in = $_y->getEncrypt($temp_search_in);
                    if(!empty($resultEdit[0]->$key)){
                        $relatin_query = $table_fields[$key]['query'] . ' where '.$field.'='.$resultEdit[0]->$key.';';
                        ...
                        if(!empty($value_name[0]->name)){
                            ...
                            $default_valor_last_name = !empty($value_name[0]->last_name) ? ' '.$value_name[0]->last_name : '';
                            $default_valor_name = $value_name[0]->first_name . $default_valor_last_name;
                        }
                    }
                }elseif($field_type_of=='seekerequal'){
                    $_y = new Crypt();
                    $temp_search_in = base64_encode(serialize($field_foreign_fields));
                    $_y->setKey(CRYPT_VAR_TXT);
                    $search_in = $_y->getEncrypt($temp_search_in);
                    if(!empty($resultEdit[0]->$key)){
                        $where_field = str_replace('equal1_','',$field);
                        $where_field = str_replace('equal2_','',$where_field);
                        $where_field = str_replace('equal3_','',$where_field);
                        $relatin_query = $table_fields[$key]['query'] . ' where '.$where_field.'='.$resultEdit[0]->$key.';';
                        // print_r($relatin_query);
                        $value_name = $this->_model->getInjection($relatin_query);
                        if(!empty($value_name[0]->name)){
                            $default_valor_name = $value_name[0]->name;
                        }elseif(!empty($value_name[0]->first_name)){
                    ...
                    // echo $field_type_of . '|' . $field_primary_key . '|' . $field . '|' . $table_fields[$key]['query'] . '<br>';
            ...
            if($field_type_of=='varchartoken' && empty($resultEdit[0]->$key)){
                // $default_valor = 'asdsdassdasdfbdbad';
                $theType->setToken($_flash_token);
            }else{
                $default_valor = $resultEdit[0]->$key;
            }
...
$html_main .= '              <form action="index.php" method="post" name="' . $this->_controller . '" id="' . $this->_controller . '" enctype="multipart/form-data" class="form-horizontal">';
$html_main .= '                  <input type="hidden" name="controller" value="'.$this->_controller.'" />';
$html_main .= '                  <input type="hidden" name="action" value="talos02" />';
$html_main .= '                  <input type="hidden" name="pager_page" value="'.$this->_pager_page.'" />';
$html_main .= '                  <input type="hidden" name="'.$id_record_name.'" value="'.$id_record_value.'" />';
...
$ckeditor_script = '
    ...
    CKEDITOR.config.format_tags = "p;h1;h2;h3;h4;h5;h6;pre;address;div";
    CKEDITOR.config.colorButton_backStyle = { element: \'tr\', styles: { \'background-color\': \'#(color)\' }};
...
// $temp->addFile("body",0,"script",array('type'=>'text/javascript', 'src'=>CONFIG_HOST_NAME_BACKEND.FOLDER_THEME.TEMPLATE_SKIN_BACKEND.'/js/ckeditor/new/ckeditor.js'));
