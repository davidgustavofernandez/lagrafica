...
$urlExportBatchList = 'index.php?controller='.$this->_controller.'&action=exportBatchList&';
...
switch($action){
  ...
  case 'listed':
    // if($this->_controller=='patients_evolutions'){
    //   require_once( dirname(__FILE__) . '/_actions/listed_simple.php' );
    // }else{
    ...
    if( is_array($result) )
    {
      ...
      if( !empty($result['0']))
      {
        foreach($result as $retorno){
          if(isset($retorno->name) && isset($retorno->last_name)){
            $patients_array[] = array( "value" => $retorno->first_name.' '.$retorno->last_name, "data" => $retorno->$the_key );
          }elseif(isset($retorno->first_name) && isset($retorno->last_name)){
            $patients_array[] = array( "value" => $retorno->first_name.' '.$retorno->last_name, "data" => $retorno->$the_key );
          }elseif(isset($retorno->name)){
            $patients_array[] = array( "value" => $retorno->name, "data" => $retorno->$the_key );
          }elseif(isset($retorno->first_name)){
            $patients_array[] = array( "value" => $retorno->first_name, "data" => $retorno->$the_key );
          }
      ...
      echo json_encode(array(
        "query" => "Unit",
        "suggestions"   => $patients_array
      ));
      exit();
    }
    else
    {
      $patients_array[] = array( "value" => 'Sin Coincidencia', "data" => 'Sin Coincidencia' );
      echo json_encode(array(
        "query" => "Unit",
        "suggestions" => $patients_array,
      ));
  ...
  case 'exportBatch':
    require_once( dirname(__FILE__) . '/_actions/exportBatch.php' );
  break;

  case 'exportBatchList':
    require_once( dirname(__FILE__) . '/_actions/exportBatchList.php' );
	break;

	case 'exportBatchFileDelete':
		header('Location: '.utf8_encode($urlExportBatchList).'');
		exit();
	break;

	case 'exportBatchEraseAllFiles':
		header('Location: '.utf8_encode($urlExportBatchList).'');
		exit();
	break;
